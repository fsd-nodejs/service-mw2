 ...
 * - 对异常链路进行上报
 */
async function traceMiddleware(ctx: Context, next: IMidwayWebNext): Promise<unknown> {
  // 白名单内的路由不会被追踪
  ...
  const requestSpanCtx = globalTracer().extract(FORMAT_HTTP_HEADERS, ctx.headers) ?? undefined
